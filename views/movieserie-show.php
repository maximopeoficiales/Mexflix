...
if( $_POST['r'] == 'movieserie-show' && isset($_POST['imdb_id']) ) {
    ...
    /* si esta vacio osea no se pudo cargar la informacion */
	if( empty($ms) ) {
		printf('
			<div class="container">
        ...
        ';
        /* str_replace (que busco,lo reempleazo con, parametro) */
		...
		printf(
			...
			$ms[0]['premiere'],
			$ms[0]['rating'],
			$ms[0]['poster'],   /* estas fallidos los links */
			$ms[0]['author'],   
			$ms[0]['actors'],
			$ms[0]['plot'],
			$trailer
        );
        /* por eso paso como parametro trailer */
	}
} else {
